...
const { setMaintenanceWindow, setMaintenanceTime, getMaintenanceTime } = require('../maintenance-window')
...
async function main (fabricator, debugOut, argv) {
  ...
  const onCluster = async (tokens, cluster) => {
    ...
    // make sure the cluster has a PX00 mainenance window channel, and the
    ...
    const win = channels.find(c => c.match(/P\d00/))
    log.debug(`cluster is in ${win} maintenance channel`)
    if (win) {
      const existingTime = getMaintenanceTime(clusterClient, cluster)
      log.info('ensuring maintenance window startTime is correct..')
      if (maintenanceWindows[win].startTime === existingTime) return
      return setMaintenanceTime(clusterClient, cluster, maintenanceWindows[win].startTime)
    }
    const windowChannel = await setMaintenanceWindow(clusterClient, maintenanceWindows, cluster)
